...
namespace UI.Settings
{
    ...
    public class SettingsVectorInspector : SettingsValueInspector
    {
        ...
        public void OnEndEdit()
        {
            ...
            {
                Vector3 v = new Vector3(float.Parse(m_XInputField.text), float.Parse(m_YInputField.text), float.Parse(m_ZInputField.text));
                m_Wrapper.Value = v;
            }
            else if (m_Wrapper.Type == typeof( Vector4 ))
            {
                Vector4 v = new Vector4(float.Parse(m_XInputField.text), float.Parse(m_YInputField.text), float.Parse(m_ZInputField.text), float.Parse(m_WInputField.text));
                m_Wrapper.Value = v;
            }
            else
            {
