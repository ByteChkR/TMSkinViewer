...
namespace S16.Drawing
{
    ...
    public class DDSImage
    {
        ...
        /// </summary>
        public Texture2D BitmapImage => m_bitmap;
        ...
        private bool Check16BitComponents( DDSStruct header )
        {
            ...
            // a2b10g10r10 format
            if ( header.pixelformat.rbitmask == 0x3FF00000 &&
            ...
            if ( header.pixelformat.rbitmask == 0x000003FF &&
                 header.pixelformat.gbitmask == 0x000FFC00 &&
                ...
                for ( int z = 0; z < depth; z++ )
                {
                    for ( int y = 0; y < height; y += 4 )
                    {
                        for ( int x = 0; x < width; x += 4 )
                        {
                            ...
                            {
                                // First three bytes
                                uint bitmask = ( ( uint )temp[0] << 0 ) |
                                               ( ( uint )temp[1] << 8 ) |
                                               ( ( uint )temp[2] << 16 );

                                uint bitmask2 = ( ( uint )temp2[0] << 0 ) |
                                                ( ( uint )temp2[1] << 8 ) |
                                                ( ( uint )temp2[2] << 16 );
        ...
        private unsafe byte[] DecompressARGB( DDSStruct header, byte[] data, PixelFormat pixelFormat )
        {
            ...
            int sizeOfData =
                ( int )( header.width * header.pixelformat.rgbbitcount / 8 * header.height * header.depth );
            ...
            fixed ( byte* bytePtr = data )
            {
                ...
                for ( int i = 0; i < sizeOfData; i += bpp )
                {
                    ...
                    rawData[i] = ( byte )( ( ( ( int )readI & ( int )header.pixelformat.rbitmask ) >> ( int )redR ) <<
                                           ( int )redL );

                    if ( bpp >= 3 )
                    {
                        rawData[i + 1] =
                            ( byte )( ( ( ( int )readI & ( int )header.pixelformat.gbitmask ) >> ( int )greenR ) <<
                                      ( int )greenL );

                        rawData[i + 2] = ( byte )( ( ( ( int )readI & header.pixelformat.bbitmask ) >> ( int )blueR ) <<
                                                   ( int )blueL );

                        if ( bpp == 4 )
                        {
                            rawData[i + 3] =
                                ( byte )( ( ( ( int )readI & ( int )header.pixelformat.alphabitmask ) >>
                                            ( int )alphaR ) <<
                                          ( int )alphaL );
            ...
            fixed ( byte* bytePtr = data )
            {
                ...
                fixed ( byte* destPtr = rawData )
                {
                    ...
                    for ( int i = 0; i < sizeOfData / 2; i += bpp )
                    {
                        ...
                        ( ( ushort* )destData )[i + 2] =
                            ( ushort )( ( ( ( int )readI & ( int )header.pixelformat.rbitmask ) >> ( int )redR ) <<
                                        ( int )redL );

                        if ( bpp >= 3 )
                        {
                            ( ( ushort* )destData )[i + 1] =
                                ( ushort )( ( ( ( int )readI & ( int )header.pixelformat.gbitmask ) >>
                                              ( int )greenR ) <<
                                            ( int )greenL );
                            ...
                            {
                                ( ( ushort* )destData )[i + 3] =
                                    ( ushort )( ( ( ( int )readI & ( int )header.pixelformat.alphabitmask ) >>
                                                  ( int )alphaR ) <<
                                                ( int )alphaL );
                                ...
                                {
                                    ( ( ushort* )destData )[i + 3] =
                                        ( ushort )( ( ( ushort* )destData )[i + 3] |
                                                    ( ( ( ushort* )destData )[i + 3] >> 4 ) );
                                }
                            }
                        }
                        else if ( bpp == 2 )
                        {
                            ...
                            {
                                ( ( ushort* )destData )[i + 1] =
                                    ( ushort )( ( ( ushort* )destData )[i + 1] != 0 ? 0xFF : 0x00 );
                            }
                            else if ( alphaL >= 4 )
                            {
                                ( ( ushort* )destData )[i + 1] =
                        ...
                        for ( int x = 0; x < width; x += 4 )
                        {
                            ...
                            for ( int j = 0; j < 4; j++ )
                            {
                                ...
                                for ( int i = 0; i < 4; i++ )
                                {
                                    ...
                                    {
                                        uint offset =
                                            ( uint )( z * sizeofplane + ( y + j ) * bps + ( x + i ) * bpp + 3 );
        ...
        private unsafe byte[] DecompressRGB( DDSStruct header, byte[] data, PixelFormat pixelFormat )
        {
            ...
            uint valMask = ( uint )( header.pixelformat.rgbbitcount == 32
                                         ? ~0
