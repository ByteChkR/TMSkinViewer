...
public class NVWaterShaders : MonoBehaviour
{
    ...
    private Camera currentCamera, reflectionCamera;
    ...
    private Vector2 wVectorX, wVectorY;
    ...
    private void LateUpdate()
    {
        ...
        {
            wVectorX = Quaternion.AngleAxis( Time.time * rotateSpeed.x, Vector3.forward ) *
                       Vector2.one *
                       rotateDistance.x;

            wVectorY = Quaternion.AngleAxis( Time.time * rotateSpeed.y, Vector3.forward ) *
                       Vector2.one *
                       rotateDistance.y;

            if ( windZone != null )
            {
                windZone.rotation = Quaternion.LookRotation( new Vector3( wVectorY.x, 0, wVectorY.y ), Vector3.zero ) *
    ...
    private void MirrorReflection()
    {
        ...
        if ( !enabled || !thisRenderer.enabled || !thisRenderer || !thisRenderer.sharedMaterial )
        {
            return;
        }
        ...
        {
            if ( mat.HasProperty( "_MirrorReflectionTex" ) )
            {
                mat.SetTexture( "_MirrorReflectionTex", reflectionTexture );
            }
        }
