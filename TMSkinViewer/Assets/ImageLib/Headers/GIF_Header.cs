    ...
    public class GIF_Header : AbstractHeader
    {
        ...
        private const int
            GlobalHeaderSize =
                ...
                768 +
                19 +
                8; // Header block, Logical Screen Descriptor, Max size Global Colour Table, Application Extension (animation), Graphics Control Extension (Transparency)
        ...
        ///     Interlacing is showing every other line, then filling in the blanks later so user gets an idea what it looks like
        ///     first.
        /// </summary>
        ...
        ///     Image format.
        /// </summary>
        public override ImageEngineFormat Format => ImageEngineFormat.GIF;
        ...
        protected override long Load( Stream stream )
        {
            ...
            if ( HasGlobalColourTable )
            {
                ...
                {
                    Console.WriteLine(
                                      $"Global table size incorrect in header. Header = {GlobalColourTableSize}, should be {estimatedSize}"
                                     );
                }
                ...
                if ( temp[0] == 0x21 ) // Extension indicator
                {
                    ...
                    else
                    {
                        stream.Seek(
                                    -18,
                                    SeekOrigin.Current
                                   ); // Skip ignored optionals ideally, BUT this could also be false match i.e. not an extension block, so we can only skip that identifier and keep looking.
                    }
                }
                else
                {
