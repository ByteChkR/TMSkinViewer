...
namespace CSharpImageLibrary.Headers
{
    ...
    public class DDS_Header : AbstractHeader
    {
        ...
        public enum D3D10_RESOURCE_DIMENSION
        {
            ...
            /// <summary>
            ///     1D Texture specified by dwWidth of DDS_Header = size of texture. Typically, dwHeight = 1 and DDSD_HEIGHT flag is
            ///     also set in dwFlags.
            /// </summary>
            DDS_DIMENSION_TEXTURE1D = 2,

            /// <summary>
            ///     2D Texture specified by dwWidth and dwHeight of DDS_Header. Can be cube map if miscFlag and arraySize members are
            ///     set.
            /// </summary>
        ...
        public enum DDS_PFdwFlags
        {
            ...
            /// <summary>
            ///     Used in some older files for alpha channel only uncompressed data. i.e. dwRGBBitCount contains alpha channel
            ...
            /// <summary>
            ///     Used in some old files for YUV uncompressed data. i.e. dwRGBBitCount contains YUV bitcount, dwRBitMask contains Y
            ///     mask, dwGBitMask contains U mask, dwBBitMask contains V mask.
            ///     YUV is a weird colourspace. Y = intensity, UV = colour. Y = 0-1 (0-255), U,V = -0.5-0.5 (-128-127) or 0-255.
            /// </summary>
            DDPF_YUV = 0x200,

            /// <summary>
            ///     Old flag for single channel colour uncompressed. dwRGBBitCount contains luminescence channel bit count, dwRBitMask
            ///     contains channel mask. Can combine with DDPF_ALPHAPIXELS for 2 channel DDS file.
            /// </summary>
            DDPF_LUMINANCE = 0x20000, // Older flag for single channel uncompressed data

            /// <summary>
            ///     Undocumented flag that seems to indicate that format is signed. EDIT Seems to be a nVidia thing as specified in the
            ///     NVTT solution: https://github.com/castano/nvidia-texture-tools
            /// </summary>
            DDPF_SIGNED = 0x80000,
        ...
        public enum DXGI_MiscFlags
        {
            ...
            /// <summary>
            ///     Any alpha channels are premultiplied i.e. RGB (without A) with premultiplied alpha has values that include alpha by
            ///     way of multiplying the original RGB with the A.
            /// </summary>
        ...
        public struct DDS_DXGI_DX10_Additional
        {
            ...
            {
                return General.StringifyObject( this, true );
            }
        ...
        private const int MaxHeaderSize = ImageFormats.DDS_DX10_HEADER_LENGTH;

        private ImageEngineFormat format = ImageEngineFormat.Unknown;
        ...
        public DDS_Header(
            ...
            int Height,
            int Width,
            ImageEngineFormat surfaceformat,
            DXGI_FORMAT dx10Format = DXGI_FORMAT.DXGI_FORMAT_UNKNOWN )
        {
            dwSize = 124;

            dwFlags = DDSdwFlags.DDSD_CAPS |
                      DDSdwFlags.DDSD_HEIGHT |
                      DDSdwFlags.DDSD_WIDTH |
                      DDSdwFlags.DDSD_PIXELFORMAT |
                      ( Mips != 1 ? DDSdwFlags.DDSD_MIPMAPCOUNT : 0 );

            this.Width = Width;
            this.Height = Height;

            dwCaps = DDSdwCaps.DDSCAPS_TEXTURE |
                     ( Mips == 1 ? 0 : DDSdwCaps.DDSCAPS_COMPLEX | DDSdwCaps.DDSCAPS_MIPMAP );
                ...
                DX10_DXGI_AdditionalHeader = new DDS_DXGI_DX10_Additional
                                             {
                                                 ...
                                                 miscFlags2 =
                                                     DXGI_MiscFlags.DDS_ALPHA_MODE_UNKNOWN,
                                                 arraySize = 1
                                             };
            }
        }
        ...
        public static ImageEngineFormat DetermineDDSSurfaceFormat( DDS_PIXELFORMAT ddspf )
        {
            ...
            if ( format == ImageEngineFormat.Unknown )
            {
                ...
                // KFreon: G_R only.
                else if ( ( ddspf.dwFlags & DDS_PFdwFlags.DDPF_RGB ) == DDS_PFdwFlags.DDPF_RGB &&
                          !( ( ddspf.dwFlags & DDS_PFdwFlags.DDPF_ALPHAPIXELS ) == DDS_PFdwFlags.DDPF_ALPHAPIXELS ) &&
                          ddspf.dwABitMask == 0 &&
                          ddspf.dwBBitMask == 0 &&
                          ddspf.dwGBitMask != 0 &&
                          ddspf.dwRBitMask != 0 )
                ...
                else if ( ( ddspf.dwFlags & ( DDS_PFdwFlags.DDPF_RGB | DDS_PFdwFlags.DDPF_ALPHAPIXELS ) ) ==
                          ( DDS_PFdwFlags.DDPF_RGB | DDS_PFdwFlags.DDPF_ALPHAPIXELS ) ||
                          ddspf.dwABitMask != 0 &&
                          ddspf.dwBBitMask != 0 &&
                          ddspf.dwGBitMask != 0 &&
                          ddspf.dwRBitMask != 0 )
        ...
        {
            base.Load( stream );
            byte[] temp = stream.ReadBytes( MaxHeaderSize );
        ...
        private static FourCC ParseFormatToFourCC( ImageEngineFormat format )
        {
        ...
        /// <returns>Friendly format.</returns>
        private static ImageEngineFormat ParseFourCC(
            FourCC fourCC,
            DXGI_FORMAT additionalDX10 = DXGI_FORMAT.DXGI_FORMAT_UNKNOWN )
        {
        ...
        private string DDS_FlagStringify( Type enumType )
        {
            string flags = "";
