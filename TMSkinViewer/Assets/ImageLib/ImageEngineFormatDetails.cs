...
namespace CSharpImageLibrary
{
    ...
    public partial class ImageFormats
    {
        ...
        public class ImageEngineFormatDetails
        {
            ...
            /// </summary>
            public Action < byte[], int, int, byte[], int, AlphaSettings, ImageEngineFormatDetails >
                BlockEncoder = null;
            ...
            private Func < byte[], int, byte[] > ReadUShortAsArray = null;
            private Func < byte[], int, byte[] > ReadFloatAsArray = null;
            ...
            public int HeaderSize =>
                ...
                Format == ImageEngineFormat.DDS_ARGB_32F &&
                DX10Format == DDS_Header.DXGI_FORMAT.DXGI_FORMAT_R32G32B32A32_FLOAT
                    ? DDS_DX10_HEADER_LENGTH
                    : DDS_NO_DX10_HEADER_LENGTH;
            ...
            public bool IsPremultipliedFormat =>
                Format == ImageEngineFormat.DDS_DXT2 || Format == ImageEngineFormat.DDS_DXT4;
            ...
            public int ComponentSize
            {
                get
                {
                    int aveChannelWidth = BitCount / 8;
                    int remainder = aveChannelWidth % MaxNumberOfChannels;
            ...
            /// <summary>
            ///     Size of a discrete block in bytes. (e.g. 2 channel 8 bit colour = 2, DXT1 = 16). Block can mean texel (DXTn) or
            ///     pixel (uncompressed)
            /// </summary>
            ...
            public SupportedExtensions Supported_Extension =>
                IsDDS ? SupportedExtensions.DDS : ParseExtension( Format.ToString() );
            ...
            public ImageEngineFormatDetails(
                ImageEngineFormat inFormat,
                DDS_Header.DXGI_FORMAT dxgiFormat = new DDS_Header.DXGI_FORMAT() )
            {
                Format = inFormat;
            ...
            private int GetDX10BitCount( DDS_Header.DXGI_FORMAT DX10Format )
            {
                int dx10Format = 32;
            ...
            private byte ReadByteFromByte( byte[] source, int sourceStart )
            {
                return source[sourceStart];
            }

            private byte ReadByteFromFloat( byte[] source, int sourceStart )
            {
                return ( byte )( BitConverter.ToSingle( source, sourceStart ) * 255f );
            }

            private byte ReadByteFromUShort( byte[] source, int sourceStart )
            {
                return ( byte )( ( ( source[sourceStart + 1] << 8 ) | source[sourceStart] ) *
                                 ( 255d / ushort.MaxValue ) );
            }

            /************************************/

            private float ReadFloatFromByte( byte[] source, int sourceStart )
            {
            ...
            private byte[] ReadFloatFromByteOrUShortAsArray( byte[] source, int sourceStart )
            {
                return BitConverter.GetBytes( ReadFloat( source, sourceStart ) );
            }

            private float ReadFloatFromFloat( byte[] source, int sourceStart )
            {
                return BitConverter.ToSingle( source, sourceStart );
            }

            private byte[] ReadFloatFromFloatAsArray( byte[] source, int sourceStart )
            {
                return new byte[]
                       {
                           source[sourceStart],
                           source[sourceStart + 1],
                           source[sourceStart + 2],
                           source[sourceStart + 3]
                       };
            }

            private float ReadFloatFromUShort( byte[] source, int sourceStart )
            {
            ...
            private ushort ReadUShortFromByte( byte[] source, int sourceStart )
            {
                return source[sourceStart];
            }

            private byte[] ReadUShortFromByteAsArray( byte[] source, int sourceStart )
            {
                return new byte[] { 0, source[sourceStart] };
            }

            private ushort ReadUShortFromFloat( byte[] source, int sourceStart )
            {
                return ( ushort )( BitConverter.ToSingle( source, sourceStart ) * ushort.MaxValue );
            }

            private byte[] ReadUShortFromFloatAsArray( byte[] source, int sourceStart )
            {
                return BitConverter.GetBytes( ReadUShortFromFloat( source, sourceStart ) );
            }

            private ushort ReadUShortFromUShort( byte[] source, int sourceStart )
            {
                return BitConverter.ToUInt16( source, sourceStart );
            }

            private byte[] ReadUShortFromUShortAsArray( byte[] source, int sourceStart )
            {
            ...
            private void WriteByte(
                byte[] source,
                int sourceStart,
                ImageEngineFormatDetails sourceFormatDetails,
                byte[] destination,
                int destStart )
            {
            ...
            private void WriteByteMax( byte[] source, int sourceStart )
            {
            ...
            private void WriteFloatMax( byte[] source, int sourceStart )
            {
            ...
            private void WriteUShortMax( byte[] source, int sourceStart )
            {
                source[sourceStart] = 255;
