...
namespace CSharpImageLibrary
{
    ...
    public class ImageEngineImage : IDisposable
    {
        ...
        /// </summary>
        public int NumMipMaps => MipMaps.Count;

        /// <summary>
        ///     Format of image.
        /// </summary>
        public ImageEngineFormat Format => FormatDetails.Format;
        ...
        /// </summary>
        public int UncompressedSize => ImageFormats.GetUncompressedSize( Width, Height, NumberOfChannels, false );

        /// <summary>
        ///     Number of channels in image.
        ///     NOTE: Still stored in memory as BGRA regardless.
        /// </summary>
        public int NumberOfChannels => FormatDetails.MaxNumberOfChannels;
        ...
        public ImageEngineImage( byte[] imageData, int maxDimension = 0 )
        {
            using ( MemoryStream
                ms = new MemoryStream(
                                      ...
                                      false,
                                      true
                                     ) ) // Need to be able to access underlying byte[] using <Stream>.GetBuffer()
            {
        ...
        {
            MipMap top = MipMaps[0];
            int determiningDimension = top.Width > top.Height ? top.Width : top.Height;
        ...
        /// <param name="removeAlpha">
        ///     True = Alpha removed. False = Uses threshold value and alpha values to mask RGB FOR DXT1
        ///     ONLY, otherwise removes completely.
        /// </param>
        ...
        public async Task Save(
            ...
            MipHandling GenerateMips,
            int desiredMaxDimension = 0,
            int mipToSave = 0,
            bool removeAlpha = true )
        {
            byte[] data = Save( destFormatDetails, GenerateMips, desiredMaxDimension, mipToSave, removeAlpha );
        ...
        /// <param name="removeAlpha">
        ///     True = removes alpha. False = Uses threshold value and alpha values to mask RGB FOR DXT1
        ///     ONLY, otherwise removes completely.
        /// </param>
        public void Save(
            Stream destination,
            ...
            bool removeAlpha = true )
        {
            ...
            else if ( destFormatDetails.Format == ImageEngineFormat.DDS_DXT2 ||
                      destFormatDetails.Format == ImageEngineFormat.DDS_DXT4 )
            {
            ...
            {
                return AttemptSaveUsingOriginalData(
                                                    destFormatDetails,
                                                    GenerateMips,
                                                    desiredMaxDimension,
                                                    mipToSave,
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine( $"Width x Height: {Width}x{Height}" );
            sb.AppendLine( $"Num Mips: {NumMipMaps}" );
        ...
        private byte[] AttemptSaveUsingOriginalData(
            ImageFormats.ImageEngineFormatDetails destFormatDetails,
            MipHandling GenerateMips,
            ...
            AlphaSettings alphaSetting )
        {
            ...
            if ( destFormatDetails.IsDDS )
            {
                ...
                {
                    int index = MipMaps.FindIndex(
                                                  t => t.Width < desiredMaxDimension && t.Height < desiredMaxDimension
                                                 );
                ...
                else
                {
                    ...
                    switch ( GenerateMips )
                    {
                        ...
                        case MipHandling.GenerateNew:
                            ...
                            byte[] formattedMips = DDSGeneral.Save(
                                                                   MipMaps.GetRange( 1, MipMaps.Count - 1 ),
                                                                   destFormatDetails,
                                                                   alphaSetting
                                                                  );
                            ...
                            length = ImageFormats.GetCompressedSize(
                                                                    0,
                                                                    destFormatDetails,
                                                                    newWidth,
                                                                    newHeight
                                                                   ); // Should be the length of the top mipmap.
                            ...
                            Array.Copy(
                                       formattedMips,
                                       destFormatDetails.HeaderSize,
                                       data,
                                       length,
                                       formattedMips.Length - destFormatDetails.HeaderSize
                                      );
        ...
        private void Load( Stream stream, int maxDimension )
        {
            CompressedSize = ( int )stream.Length;
            Header = ImageEngine.LoadHeader( stream );

            // DX10
            DDS_Header.DXGI_FORMAT DX10Format = DDS_Header.DXGI_FORMAT.DXGI_FORMAT_UNKNOWN;

            if ( Header is DDS_Header )
            {
                DX10Format = ( ( DDS_Header )Header ).DX10_DXGI_AdditionalHeader.dxgiFormat;
            }

            ImageEngineFormat tempFormat = Header.Format;

            if ( DX10Format ==
                 DDS_Header.DXGI_FORMAT.
                            DXGI_FORMAT_R32G32B32A32_FLOAT ) // Trickses to get around the DX10 float header deal - Apparently float formats should be specified with the DX10 header...
            {
                tempFormat = ImageEngineFormat.DDS_ARGB_32F;
                DDS_Header.DDS_PIXELFORMAT tempPF = ( ( DDS_Header )Header ).ddspf;
                tempPF.dwRBitMask = 1;
                tempPF.dwGBitMask = 2;
                tempPF.dwBBitMask = 3;
                tempPF.dwABitMask = 4;
