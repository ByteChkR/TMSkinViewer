...
namespace CSharpImageLibrary.DDS
{
    ...
    internal static class DDS_Decoders
    {
        ...
        // BC4
        internal static void DecompressATI1Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            ...
            bool unused )
        {
            DDS_BlockHelpers.Decompress8BitBlock(
                                                 source,
                                                 sourceStart,
                                                 destination,
                                                 decompressedStart,
        ...
        // BC5
        internal static void DecompressATI2Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
        ...
        internal static void DecompressBC1Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
        ...
        {
            DDS_BlockHelpers.DecompressRGBBlock(
                                                source,
                                                sourceStart,
                                                destination,
                                                decompressedStart,
        ...
        internal static void DecompressBC2Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
            ...
            bool isPremultiplied )
        {
            ...
            for ( int i = 0; i < 8; i++ )
            {
                ...
                destination[offset] =
                    ( byte )( source[sourceStart + i] &
                              0xF0 ); // Decompress doesn't need formatDetails as it's just reading raw.
        ...
        internal static void DecompressBC3Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
        ...
        // BC6
        internal static void DecompressBC6Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
            ...
            bool unused )
        {
            DX10_Helpers.LDRColour[] colours = BC6.DecompressBC6( source, sourceStart, false );
            SetColoursFromDX10( colours, destination, decompressedStart, decompressedLineLength );
        }

        // BC7
        internal static void DecompressBC7Block(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
            ...
            bool unused )
        {
            DX10_Helpers.LDRColour[] colours = BC7.DecompressBC7( source, sourceStart );
            SetColoursFromDX10( colours, destination, decompressedStart, decompressedLineLength );
        }
        ...
        {
            bool requiresSignedAdjustment = ( ddspf.dwFlags & DDS_Header.DDS_PFdwFlags.DDPF_SIGNED ) ==
                                            DDS_Header.DDS_PFdwFlags.DDPF_SIGNED;

            int sourceIncrement = ddspf.dwRGBBitCount / 8; // /8 for bits to bytes conversion

            bool oneChannel = ( ddspf.dwFlags & DDS_Header.DDS_PFdwFlags.DDPF_LUMINANCE ) ==
                              DDS_Header.DDS_PFdwFlags.DDPF_LUMINANCE;

            bool twoChannel =
                ( ddspf.dwFlags & DDS_Header.DDS_PFdwFlags.DDPF_ALPHAPIXELS ) ==
                ...
                case 1:
                    ...
                    if ( maskOrder.Count != sourceIncrement )
                    {
                        ...
                        ReadBytesLegacy(
                                        ...
                                        lengths,
                                        destination,
                                        new int[] { destBInd, destGInd, destRInd, destAInd }
                                       );
                    }
                    else
                    {
                        ReadBytes(
                                  ...
                                  sourceStart,
                                  sourceIncrement,
                                  new int[] { BIndex, GIndex, RIndex, AIndex },
                                  destination,
                                  new int[] { destBInd, destGInd, destRInd, destAInd }
                                 );
                    }
        ...
        private static int CountSetBits( uint i )
        {
            i = i - ( ( i >> 1 ) & 0x5555_5555 );
        ...
        private static byte ExpandTo255( double v )
        {
        ...
        private static void ReadBytesLegacy(
            byte[] source,
            int sourceStart,
            int sourceIncrement,
            int[] lengths,
        ...
        private static void ReadUShorts(
            byte[] source,
            int sourceStart,
            int sourceIncrement,
            int[] sourceInds,
        ...
        private static void SetColoursFromDX10(
            DX10_Helpers.LDRColour[] block,
            byte[] destination,
            int decompressedStart,
            int decompressedLineLength )
        {
