...
namespace CSharpImageLibrary.DDS
{
    ...
    public static class DDSGeneral
    {
        ...
        /// <summary>
        ///     Value at which alpha is included in DXT1 conversions. i.e. pixels lower than this threshold are made 100%
        ///     transparent, and pixels higher are made 100% opaque.
        /// </summary>
        ...
        public static bool EnsureMipInImage(
            long streamLength,
            int mainWidth,
            int mainHeight,
            int desiredMipDimension,
            ...
            out int mipOffset )
        {
            ...
            if ( mipIndex < -1 )
            {
                throw new InvalidDataException(
                                               $"Invalid dimensions for mipmapping. Got desired: {desiredMipDimension} and dependent: {dependentDimension}"
                                              );
            }
            ...
            if ( destFormatDetails.Format ==
                 ImageEngineFormat.DDS_ARGB_8 ) // TODO: Might not just be 8 bit, still don't know why it's wrong.
            {
                requiredOffset -= 2;
            }
        ...
        internal static int BuildMipMaps( List < MipMap > MipMaps )
        {
            ...
            int estimatedMips = EstimateNumMipMaps( currentMip.Width, currentMip.Height );
            ...
            Action < int > action = new Action < int >(
                                                       item =>
                                                       {
                                                           ...
                                                           double scale =
                                                               ...
                                                               ( 2 <<
                                                                 ( index -
                                                                   1 ) ); // Shifting is 2^index - Math.Pow seems extraordinarly slow.
            ...
            if ( ImageEngine.EnableThreading )
            {
                Parallel.For(
                             1,
                             estimatedMips + 1,
                             new ParallelOptions { MaxDegreeOfParallelism = ImageEngine.NumThreads },
                             action
                            );
            }
            else
        ...
        internal static int GetCompressedSizeOfImage(
            int mipCount,
            ImageFormats.ImageEngineFormatDetails destFormatDetails,
            int baseWidth,
            int baseHeight )
        ...
        internal static int GetCompressedSizeUpToIndex(
            double mipIndex,
            ImageFormats.ImageEngineFormatDetails destFormatDetails,
            int baseWidth,
            int baseHeight )
        {
            ...
            */
            ...
            double sumPart =
                mipIndex == -1 ? 0 : 1d / 3d * ( 4d - shift ); // Shifting represents 4^-mipIndex. Math.Pow seems slow.

            double totalSize = destFormatDetails.HeaderSize +
                               sumPart *
                               destFormatDetails.BlockSize *
                               ( baseWidth / divisor ) *
                               ( baseHeight / divisor );
        ...
        internal static int GetMipOffset(
            double mipIndex,
            ImageFormats.ImageEngineFormatDetails destFormatDetails,
            int baseWidth,
            int baseHeight )
        ...
        internal static List < MipMap > LoadDDS(
            ...
            ImageFormats.ImageEngineFormatDetails formatDetails )
        {
            ...
            if ( desiredMaxDimension != 0 && estimatedMips > 1 )
            {
                ...
                {
                    throw new InvalidDataException(
                                                   $"Requested mipmap does not exist in this image. Top Image Size: {mipWidth}x{mipHeight}, requested mip max dimension: {desiredMaxDimension}."
                                                  );
                }
                ...
                {
                    double divisor = mipHeight > mipWidth
                                         ? mipHeight / desiredMaxDimension
                                         : mipWidth / desiredMaxDimension;
            ...
            if ( formatDetails.IsBlockCompressed ) // Threading done in the decompression, not here.
            {
                for ( int m = 0; m < estimatedMips; m++ )
                {
                    ...
                    if ( mipWidth <= 0 ||
                         mipHeight <= 0 ) // Needed cos it doesn't throw when reading past the end for some reason.
                    {
                        break;
                    }

                    MipMap mipmap = ReadCompressedMipMap(
                                                         compressed,
                                                         mipWidth,
                                                         mipHeight,
                                                         mipOffset,
                    ...
                    mipOffset +=
                        ( int )( mipWidth *
                                 mipHeight *
                                 ( blockSize /
                                   16d ) ); // Also put the division in brackets cos if the mip dimensions are high enough, the multiplications can exceed int.MaxValue)
                                                           ...
                                                           mipIndex =>
                                                           {
                                                               ...
                                                               double divisor =
                                                                   mipIndex == 0
                                                                       ...
                                                                       : 2 <<
                                                                         ( mipIndex -
                                                                           1 ); // Divisor represents 2^mipIndex - Math.Pow seems very slow.
            ...
            if ( mips.Count == 0 )
            {
                throw new InvalidOperationException(
                                                    $"No mipmaps loaded. Estimated mips: {estimatedMips}, mip dimensions: {mipWidth}x{mipHeight}"
                                                   );
            }
            ...
            AlphaSettings alphaSetting )
        {
            ...
            Action < byte[], int, int, byte[], int, AlphaSettings, ImageFormats.ImageEngineFormatDetails > compressor =
                destFormatDetails.BlockEncoder;
            ...
            {
                throw new InvalidOperationException(
                                                    $"DXT compression formats require dimensions to be multiples of 4. Got: {width}x{height}."
                                                   );
            }
            ...
            /*if (destFormatDetails.ComponentSize != 1)
                fullSize += (fullSize - headerLength) * destFormatDetails.ComponentSize;*/
            // Size adjustment for destination to allow for different component sizes.
            ...
            if ( destFormatDetails.IsBlockCompressed )
            {
                ...
                foreach ( MipMap mipmap in mipMaps )
                {
                    ...
                    int temp = WriteCompressedMipMap(
                                                     destination,
                                                     mipOffset,
                                                     mipmap,
                                                     blockSize,
                ...
                Action < int > action = new Action < int >(
                                                           mipIndex =>
                                                           {
                                                               if ( alphaSetting == AlphaSettings.RemoveAlphaChannel )
                                                               {
                                                                   ...
                                                                   DDS_Header.DDS_PIXELFORMAT ddspf = header.ddspf;
                                                                   ddspf.dwABitMask = 0;
                                                                   header.ddspf = ddspf;
                                                               }

                                                               // Get MipOffset
                                                               int offset = GetMipOffset(
                                                                    mipIndex,
                                                                    destFormatDetails,
                                                                    width,
                                                                    height
        ...
        private static MipMap ReadCompressedMipMap(
            MemoryStream compressed,
            int mipWidth,
            int mipHeight,
            int mipOffset,
            ImageFormats.ImageEngineFormatDetails formatDetails,
            Action < byte[], int, byte[], int, int, bool > DecompressBlock )
        {
                ...
                Action < int > action = new Action < int >(
                                                           texelIndex =>
                                                           {
                                                               int compressedPosition =
                                                                   mipOffset + texelIndex * formatDetails.BlockSize;

                                                               int decompressedStart =
                                                                   ( int )( texelIndex / numTexelsInRow ) *
                                                                   texelRowSkip +
                                                                   texelIndex % numTexelsInRow * 16;
                                                               ...
                                                               try
                                                               {
                                                                   DecompressBlock(
                                                                        ...
                                                                        decompressedData,
                                                                        decompressedStart,
                                                                        decompressedRowLength,
                                                                        formatDetails.IsPremultipliedFormat
                                                                       );
                                                               }
        ...
        private static MipMap ReadUncompressedMipMap(
            MemoryStream stream,
            int mipOffset,
            int mipWidth,
            int mipHeight,
            DDS_Header.DDS_PIXELFORMAT ddspf,
            ImageFormats.ImageEngineFormatDetails formatDetails )
        {
        ...
        private static int WriteCompressedMipMap(
            byte[] destination,
            int mipOffset,
            MipMap mipmap,
            int blockSize,
            ...
            AlphaSettings alphaSetting )
        {
            ...
            Action < int > mipWriter = new Action < int >(
                                                          texelIndex =>
                                                          {
                                                              ...
                                                              int sourceLineOffset =
                                                                  ...
                                                                  4 *
                                                                  ( texelIndex /
                                                                    numTexelsInLine ); // Length in bytes x 3 lines x texel line index (how many texel sized lines down the image are we). Index / width will truncate, so for the first texel line, it'll be < 0. For the second texel line, it'll be < 1 and > 0.

                                                              int sourceTopLeftCorner =
                                                                  texelIndex %
                                                                  numTexelsInLine *
                                                                  16 *
                                                                  mipmap.LoadedFormatDetails.ComponentSize +
                                                                  sourceLineOffset; // *16 since its 4 pixels with 4 channels each. Index % numTexels will effectively reset each line.
                                                              ...
                                                              compressor(
                                                                         ...
                                                                         sourceLineLength,
                                                                         destination,
                                                                         mipOffset + texelIndex * blockSize,
                                                                         alphaSetting,
                                                                         mipmap.LoadedFormatDetails
                                                                        );
                                                          }
                                                         );
        ...
        private static void WriteUncompressedMipMap(
            byte[] destination,
            int mipOffset,
            MipMap mipmap,
            ImageFormats.ImageEngineFormatDetails destFormatDetails,
            DDS_Header.DDS_PIXELFORMAT ddspf )
        {
            DDS_Encoders.WriteUncompressed(
                                           mipmap.Pixels,
                                           destination,
                                           mipOffset,
                                           ddspf,
