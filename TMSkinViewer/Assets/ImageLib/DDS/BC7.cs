    ...
    internal static class BC7
    {
        ...
        private struct Mode
        {
            ...
            public Mode(
                ...
                int partitionBits,
                int IndexPrecision,
                LDRColour rawrgbPrecision,
                LDRColour RGBPrecisionWithP,
                int APrecision,
                int PBits,
                int RotationBits,
                int IndexModeBits )
        ...
        private static Mode[] Modes = new Mode[]
                                      {
                                          /* Mode 0: */
                                          new Mode(
                                                   0,
        ...
        public static LDRColour[] DecompressBC7( byte[] source, int sourceStart )
        {
            ...
            LDRColour[] outColours = new LDRColour[NUM_PIXELS_PER_BLOCK];
        ...
        private static void AssignIndicies(
            ...
            int indexMode,
            Mode mode,
            LDRColourEndPointPair[] endPoints,
            int[] indicies,
            int[] alphaIndicies,
            float[] afTotalErr,
            LDRColour[] block )
            ...
            int[] bestIdxs = null )
        {
            ...
            // Vector dots
            Vector4 vPixel = new Vector4( pixelColour.R, pixelColour.G, pixelColour.B, pixelColour.A );
            Vector3 vPixel3 = new Vector3( pixelColour.R, pixelColour.G, pixelColour.B );

            if ( alphaPrecision == 0 )
            {
                for ( int i = 0; i < numIndicies && bestError > 0; i++ )
                {
                    Vector4 tPixel = new Vector4( palette[i].R, palette[i].G, palette[i].B, palette[i].A );
            ...
            else
            {
                for ( int i = 0; i < numIndicies && bestError > 0; i++ )
                {
                    Vector3 tPixel = new Vector3( palette[i].R, palette[i].G, palette[i].B );
            ...
            int[] alphaIndicies )
        {
            ...
            int[] prec = new int[BC7_NUM_CHANNELS]
                         {
                             mode.RawRGBPrecision.R,
                             mode.RawRGBPrecision.G,
                             mode.RawRGBPrecision.B,
                             mode.RawRGBPrecision.A
                         };

            int[] precP = new int[BC7_NUM_CHANNELS]
                          {
                              mode.RGBPrecisionWithP.R,
                              mode.RGBPrecisionWithP.G,
                              mode.RGBPrecisionWithP.B,
                              mode.RGBPrecisionWithP.A
                          };
            ...
            if ( mode.PBits != 0 )
            {
                ...
                for ( int ch = 0; ch < BC7_NUM_CHANNELS; ch++ )
                {
                    ...
                    {
                        int[] A = new int[BC7_NUM_CHANNELS]
                                  {
                                      endPoints[i].A.R, endPoints[i].A.G, endPoints[i].A.B, endPoints[i].A.A
                                  };

                        int[] B = new int[BC7_NUM_CHANNELS]
                                  {
                                      endPoints[i].B.R, endPoints[i].B.G, endPoints[i].B.B, endPoints[i].B.A
                                  };
            ...
            if ( mode.APrecision != 0 )
            {
                for ( i = 0; i < NUM_PIXELS_PER_BLOCK; i++ )
                {
                    SetBits(
                            ref startBit,
                            i != 0 ? mode.APrecision : mode.APrecision - 1,
                            aI2[i],
                            destination,
                            destStart
                           );
        ...
        private static unsafe void Exhaustive(
            Mode mode,
            int ch,
            LDRColourEndPointPair endPoint,
            ref int A,
            ref int B,
            int prec,
            float fOrgErr,
        ...
        private static void GeneratePaletteQuantized(
            LDRColourEndPointPair endPoints,
            Mode mode,
            LDRColour[] palette,
            int indexMode )
        ...
        private static LDRColour Interpolate(
            LDRColour lDRColour1,
            LDRColour lDRColour2,
            int wc,
            int wa,
            int wcPrec,
            int waPrec )
        ...
        private static void OptimiseEndPoints(
            ...
            LDRColour[] block,
            float[] afOrgErr,
            LDRColourEndPointPair[] aOrgEndPoints,
            LDRColourEndPointPair[] aOptEndPoints,
            int shape )
        {
        ...
        private static void OptimiseOne(
            float fOrgErr,
            Mode mode,
            ref LDRColourEndPointPair org,
            ref LDRColourEndPointPair opt,
            int indexMode,
            LDRColour[] block,
            int np )
        {
            ...
            LDRColourEndPointPair new_a = new LDRColourEndPointPair(),
                                  new_b = new LDRColourEndPointPair(),
                                  newEndPoints = new LDRColourEndPointPair();
        ...
        private static unsafe float PerturbOne(
            Mode mode,
            int ch,
            ref LDRColourEndPointPair newEndPoints,
            ref LDRColourEndPointPair oldEndPoints,
            float oldErr,
            bool do_b,
            int indexMode,
            LDRColour[] block,
        ...
        private static LDRColour Quantize( LDRColour colour, LDRColour precision )
        {
        ...
        private static int Quantize( int c, int precision )
        {
        ...
        private static float Refine(
            int shape,
            int rotation,
            int indexMode,
            LDRColourEndPointPair[][] AllEndPoints,
            Mode mode,
            LDRColour[] block,
            byte[] destination,
            int destStart )
        ...
        private static void SetBits( ref int start, int length, int value, byte[] destination, int destStart )
        {
            if ( length == 0 )
            {
                return;
