...
namespace CSharpImageLibrary.DDS
{
    ...
    internal static class DDS_Encoders
    {
        ...
        internal static void CompressBC1Block(
            byte[] imgData,
            int sourcePosition,
            ...
            ImageFormats.ImageEngineFormatDetails formatDetails )
        {
            CompressRGBTexel(
                             ...
                             destPosition,
                             true,
                             alphaSetting == AlphaSettings.RemoveAlphaChannel ? 0 : DXT1AlphaThreshold,
                             alphaSetting,
                             formatDetails
                            );
        }

        internal static void CompressBC2Block(
            byte[] imgData,
            int sourcePosition,
            int sourceLineLength,
            byte[] destination,
            ...
            ImageFormats.ImageEngineFormatDetails formatDetails )
        {
            ...
            else
            {
                ...
                for ( int i = 0; i < 8; i += 2 )
                {
                    destination[destPosition + i] =
                        ( byte )( ( imgData[position] & 0xF0 ) | ( imgData[position + 4] >> 4 ) );

                    destination[destPosition + i + 1] =
                        ( byte )( ( imgData[position + 8] & 0xF0 ) | ( imgData[position + 12] >> 4 ) );
        ...
        internal static void CompressBC3Block(
            byte[] imgData,
            int sourcePosition,
            int sourceLineLength,
            byte[] destination,
        ...
        // ATI1
        internal static void CompressBC4Block(
            byte[] imgData,
            int sourcePosition,
            int sourceLineLength,
            byte[] destination,
        ...
        // ATI2 3Dc
        internal static void CompressBC5Block(
            byte[] imgData,
            int sourcePosition,
            int sourceLineLength,
            byte[] destination,
        ...
        internal static void WriteUncompressed(
            byte[] source,
            byte[] destination,
            int destStart,
            DDS_Header.DDS_PIXELFORMAT dest_ddspf,
            ImageFormats.ImageEngineFormatDetails sourceFormatDetails,
            ImageFormats.ImageEngineFormatDetails destFormatDetails )
        {
            int byteCount = dest_ddspf.dwRGBBitCount / 8;

            bool requiresSignedAdjust = ( dest_ddspf.dwFlags & DDS_Header.DDS_PFdwFlags.DDPF_SIGNED ) ==
                                        DDS_Header.DDS_PFdwFlags.DDPF_SIGNED;

            bool oneChannel = ( dest_ddspf.dwFlags & DDS_Header.DDS_PFdwFlags.DDPF_LUMINANCE ) ==
                              DDS_Header.DDS_PFdwFlags.DDPF_LUMINANCE;

            bool twoChannel = oneChannel &&
                              ( dest_ddspf.dwFlags & DDS_Header.DDS_PFdwFlags.DDPF_ALPHAPIXELS ) ==
                              DDS_Header.DDS_PFdwFlags.DDPF_ALPHAPIXELS;
            ...
            maskOrder.RemoveAll(
                                t => t == 0
                               ); // Required, otherwise indicies get all messed up when there's only two channels, but it's not indicated as such.
            ...
            int[] sourceInds = new int[] { sourceBInd, sourceGInd, sourceRInd, sourceAInd };
            int[] destInds = new int[] { destBIndex, destGIndex, destRIndex, destAIndex };
            uint[] masks = new uint[] { BMask, GMask, RMask, AMask };
                ...
                Parallel.For(
                             ...
                             ( ind, loopState ) =>
                             {
                                 ...
                                 WriteUncompressedPixel(
                                                        ...
                                                        masks,
                                                        destination,
                                                        destStart + ind * byteCount,
                                                        destInds,
                                                        destFormatDetails,
                                                        oneChannel,
                                                        twoChannel,
                                                        requiresSignedAdjust
                                                       );
                             }
                            );
            }
        ...
        private static void WriteUncompressedPixel(
            byte[] source,
            int sourceStart,
            int[] sourceInds,
            ImageFormats.ImageEngineFormatDetails sourceFormatDetails,
            ...
            bool oneChannel,
            bool twoChannel,
            bool requiresSignedAdjust )
        {
            if ( twoChannel ) // No large components - silly spec...
            {
                byte red = sourceFormatDetails.ReadByte( source, sourceStart );

                byte alpha = sourceFormatDetails.ReadByte(
                                                          source,
                                                          sourceStart + 3 * sourceFormatDetails.ComponentSize
                                                         );
            ...
            else if ( oneChannel ) // No large components - silly spec...
            {
                ...
                byte green = sourceFormatDetails.ReadByte(
                                                          source,
                                                          sourceStart + 1 * sourceFormatDetails.ComponentSize
                                                         );
                ...
                destination[destStart] =
                    ( byte )( blue * 0.082 +
                              green * 0.6094 +
                              blue *
                              0.3086 ); // Weightings taken from ATI Compressonator. Dunno if this changes things much.
            }
            else
            {
                ...
                for ( int i = 0; i < 4; i++ )
                {
                    ...
                    if ( mask != 0 )
                    {
                        destFormatDetails.WriteColour(
                                                      source,
                                                      sourceStart + sourceInds[i],
                                                      sourceFormatDetails,
                                                      destination,
                                                      destStart + destInds[i]
                                                     );
                    }
                }
