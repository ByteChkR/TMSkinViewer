...
namespace CSharpImageLibrary.DDS
{
    ...
    internal static class DDS_BlockHelpers
    {
        ...
        private static uint[] psteps3 = { 0, 2, 1 };
        private static uint[] psteps4 = { 0, 2, 3, 1 };

        private static RGBColour Luminance = new RGBColour( 0.2125f / 0.7154f, 1f, 0.0721f / 0.7154f, 1f );
        private static RGBColour LuminanceInv = new RGBColour( 0.7154f / 0.2125f, 1f, 0.7154f / 0.0721f, 1f );
        private const double OneThird = 1f / 3f;
        private const double TwoThirds = 2f / 3f;
        ...
        public static int[] BuildRGBPalette( int Colour0, int Colour1, bool isDXT1 )
        {
            ...
            if ( Colour0 > Colour1 )
            {
                byte r1 = ( byte )( TwoThirds * Colour0_R + OneThird * Colour1_R );
                byte g1 = ( byte )( TwoThirds * Colour0_G + OneThird * Colour1_G );
                byte b1 = ( byte )( TwoThirds * Colour0_B + OneThird * Colour1_B );

                byte r2 = ( byte )( OneThird * Colour0_R + TwoThirds * Colour1_R );
                byte g2 = ( byte )( OneThird * Colour0_G + TwoThirds * Colour1_G );
                byte b2 = ( byte )( OneThird * Colour0_B + TwoThirds * Colour1_B );
            ...
            else
            {
                ...
                byte r = ( byte )( 0.5 * Colour0_R + 0.5 * Colour1_R );
                byte g = ( byte )( 0.5 * Colour0_G + 0.5 * Colour1_G );
                byte b = ( byte )( 0.5 * Colour0_B + 0.5 * Colour1_B );
        ...
        public static void Compress8BitBlock(
            byte[] source,
            int sourcePosition,
        ...
        internal static byte[] Build8BitPalette( byte min, byte max, bool isSigned )
        {
            ...
            if ( min > max )
            {
                ...
                Colours[2] =
                    ( byte )( ( 6d * min + 1d * max ) /
                              7d ); // NO idea what the +3 is...not in the Microsoft spec, but seems to be everywhere else.
        ...
        internal static void CompressRGBTexel(
            byte[] imgData,
            int sourcePosition,
            int sourceLineLength,
            byte[] destination,
            ...
            ImageFormats.ImageEngineFormatDetails formatDetails )
        {
            ...
            {
                byte[] c2 = BitConverter.GetBytes( wColourA );
                byte[] c1 = BitConverter.GetBytes( wColourB ); ///////////////////// MIN MAX
            ...
            RGBColour[] step = DoSomethingWithPalette( uSteps, wColourA, wColourB, ColourA, ColourB );
            ...
            float fscale = wColourA != wColourB
                               ? ( uSteps - 1 ) / ( Dir.r * Dir.r + Dir.g * Dir.g + Dir.b * Dir.b )
                               : 0.0f;
            ...
            byte[] colour1 = BitConverter.GetBytes( Min );
            byte[] colour2 = BitConverter.GetBytes( Max );
            ...
            byte[] indicies = BitConverter.GetBytes( dw );
        ...
        internal static void Decompress8BitBlock(
            byte[] source,
            int sourceStart,
            byte[] destination,
            int decompressedStart,
            int decompressedLineLength,
            bool isSigned )
        {
            ...
            ulong bitmask = ( ( ulong )source[sourceStart + 2] << 0 ) |
                            ( ( ulong )source[sourceStart + 3] << 8 ) |
                            ( ( ulong )source[sourceStart + 4] <<
                              16 ) | // KFreon: Read all 6 compressed bytes into single.
                            ( ( ulong )source[sourceStart + 5] << 24 ) |
                            ( ( ulong )source[sourceStart + 6] << 32 ) |
                            ( ( ulong )source[sourceStart + 7] << 40 );
        ...
        internal static void DecompressRGBBlock(
            byte[] source,
            int sourcePosition,
            byte[] destination,
            int destinationStart,
            int destinationLineLength,
            bool isDXT1,
            bool isPremultiplied )
        {
            ...
            {
                byte bitmask =
                    source[sourcePosition + 4 + i]; // sourcePos + 4 is to skip the colours at the start of the texel.
        ...
        internal static RGBColour[] OptimiseRGB( RGBColour[] Colour, int uSteps )
        {
            ...
            for ( int iteration = 0; iteration < 8; iteration++ )
            {
                ...
                for ( int i = 0; i < Colour.Length; i++ )
                {
                    ...
                    float fDot = ( current.r - X.r ) * Dir.r +
                                 ( current.g - X.g ) * Dir.g +
                                 ( current.b - X.b ) * Dir.b;
        ...
        private static int CheckDXT1TexelFullTransparency(
            RGBColour[] texel,
            byte[] destination,
            int destPosition,
            double alphaRef )
        ...
        /// </summary>
        private static void DoColourFixErrorCorrection( RGBColour[] Colour, RGBColour[] texel )
        {
        ...
        private static uint DoOtherColourFixErrorCorrection(
            RGBColour[] texel,
            int uSteps,
            double alphaRef,
            RGBColour[] step,
            RGBColour Dir )
        {
            ...
            for ( int i = 0; i < 16; i++ )
            {
                ...
                float fdot = ( current.r - step[0].r ) * Dir.r +
                             ( current.g - step[0].g ) * Dir.g +
                             ( current.b - step[0].b ) * Dir.b;
                ...
                dw = ( iStep << 30 ) |
                     ( dw >> 2 ); // THIS  IS THE MAGIC here. This is the "list" of indicies. Somehow...
        ...
        private static void DoSomeDithering(
            RGBColour current,
            int index,
            RGBColour[] InnerColour,
            int InnerIndex,
            RGBColour[] Error )
        {
            if ( true ) // Dither
            {
                ...
                RGBColour inner = InnerColour[InnerIndex];
        ...
        private static RGBColour[] DoSomethingWithPalette(
            int uSteps,
            uint wColourA,
            uint wColourB,
            RGBColour ColourA,
        ...
        private static uint Encode565( RGBColour colour )
        {
            ...
            return ( ( uint )( temp.r * 31f + 0.5f ) << 11 ) |
                   ( ( uint )( temp.g * 63f + 0.5f ) << 5 ) |
                   ( uint )( temp.b * 31f + 0.5f );
        }
        ...
        private static RGBColour ReadColourFromTexel(
            byte[] texel,
            int i,
            bool premultiply,
            ImageFormats.ImageEngineFormatDetails formatDetails )
        {
            ...
            current.r = formatDetails.ReadFloat( texel, i + 2 * formatDetails.ComponentSize ) *
                        ( premultiply ? current.a : 1.0f );

            current.g = formatDetails.ReadFloat( texel, i + formatDetails.ComponentSize ) *
                        ( premultiply ? current.a : 1.0f );
