...
namespace CSharpImageLibrary
{
    ...
    internal class TargaImage : IDisposable
    {
        ...
        public TargaImage( TargaHeader prevHeader = null )
        {
            ...
            objTargaHeader = prevHeader ?? new TargaHeader();
            ...
            : this()
        {
            ...
            {
                throw new Exception(
                                    @"Error loading file, file '" + strFileName + "' must have an extension of '.tga'."
                                   );
            }
        }
        ...
        {
            if ( binReader != null &&
                 binReader.BaseStream != null &&
                 binReader.BaseStream.Length > 0 &&
                 binReader.BaseStream.CanSeek == true )
            {
                try
                {
                    ...
                    objTargaHeader.SetVerticalTransferOrder(
                                                            ( VerticalTransferOrder )Utilities.GetBits(
                                                                 ImageDescriptor,
                                                                 5,
                                                                 1
                                                                )
                    ...
                    objTargaHeader.SetHorizontalTransferOrder(
                                                              ( HorizontalTransferOrder )Utilities.GetBits(
                                                                   ImageDescriptor,
                                                                   4,
                                                                   1
                                                                  )
                                ...
                                for ( int i = 0; i < objTargaHeader.ColorMapLength; i++ )
                                {
                                    ...
                                    switch ( objTargaHeader.ColorMapEntrySize )
                                    {
                                        case 15:
                                            ...
                                            objTargaHeader.ColorMap.Add(
                                                                        Utilities.GetColorFrom2Bytes(
                                                                             color15[1],
                                                                             color15[0]
                                                                            )
                                                                       );
                 ...
                 binReader.BaseStream.CanSeek == true )
            {
                ...
                if ( objTargaFooter.ExtensionAreaOffset > 0 )
                {
                    try
                    {
                        ...
                        objTargaExtensionArea.SetAuthorName(
                                                            Encoding.ASCII.
                                                                     GetString(
                                                                               binReader.ReadBytes(
                                                                                    TargaConstants.
                                                                                        ExtensionAreaAuthorNameByteLength
                                                                                   )
                                                                              ).
                                                                     TrimEnd( '\0' )
                                                           );

                        objTargaExtensionArea.SetAuthorComments(
                                                                Encoding.ASCII.
                                                                         GetString(
                                                                              binReader.ReadBytes(
                                                                                   TargaConstants.
                                                                                       ExtensionAreaAuthorCommentsByteLength
                                                                                  )
                                                                             ).
                                                                         TrimEnd( '\0' )
                                                               );
                        ...
                        objTargaExtensionArea.SetJobName(
                                                         Encoding.ASCII.
                                                                  GetString(
                                                                            binReader.ReadBytes(
                                                                                 TargaConstants.
                                                                                     ExtensionAreaJobNameByteLength
                                                                                )
                                                                           ).
                                                                  TrimEnd( '\0' )
                                                        );
                        ...
                        objTargaExtensionArea.SetSoftwareID(
                                                            Encoding.ASCII.
                                                                     GetString(
                                                                               binReader.ReadBytes(
                                                                                    TargaConstants.
                                                                                        ExtensionAreaSoftwareIDByteLength
                                                                                   )
                                                                              ).
                                                                     TrimEnd( '\0' )
                                                           );
                        ...
                        string strVersionLetter = Encoding.ASCII.
                                                           GetString(
                                                                     binReader.ReadBytes(
                                                                          TargaConstants.
                                                                              ExtensionAreaSoftwareVersionLetterByteLength
                                                                         )
                                                                    ).
                                                           TrimEnd( '\0' );
                 ...
                 binReader.BaseStream.CanSeek == true )
            {
                try
                {
                    ...
                    string Signature = Encoding.ASCII.
                                                GetString(
                                                          binReader.ReadBytes(
                                                                              TargaConstants.FooterSignatureByteLength
                                                                             )
                                                         ).
                                                TrimEnd( '\0' );
                    ...
                    if ( string.Compare( Signature, TargaConstants.TargaFooterASCIISignature ) == 0 )
                    {
                        ...
                        string ResChar = Encoding.ASCII.
                                                  GetString(
                                                            binReader.ReadBytes(
                                                                                TargaConstants.
                                                                                    FooterReservedCharByteLength
                                                                               )
                                                           ).
                                                  TrimEnd( '\0' );
        ...
        private void LoadTGAImage( BinaryReader binReader )
        {
            ...
            intStride = ( ( ( int )objTargaHeader.Width * ( int )objTargaHeader.PixelDepth + 31 ) & ~31 ) >>
                        3; // width in bytes
            ...
            bmpTargaImage = new Texture2D(
                                          ( int )objTargaHeader.Width,
                                          ( int )objTargaHeader.Height
                                         );
            bmpTargaImage.SetPixelData( ImageData, 0 );
            
            // bmpTargaImage = new Bitmap(
            //                            ( int )objTargaHeader.Width,
            //                            ( int )objTargaHeader.Height,
            //                            intStride,
            //                            pf,
            ...
            //                                                 255,
            //                                                 objTargaHeader.ColorMap[i].R,
            //                                                 objTargaHeader.ColorMap[i].G,
            //                                                 objTargaHeader.ColorMap[i].B
            //                                                );
            //         }
            //
            //         else
            ...
            //     if ( objTargaHeader.PixelDepth == 8 &&
            //          ( objTargaHeader.ImageType == ImageType.UNCOMPRESSED_BLACK_AND_WHITE ||
    ...
    internal class TargaHeader
    {
        ...
        /// </summary>
        public ColorMapTypes ColorMapType => eColorMapType;
        ...
        public FirstPixelDestination FirstPixelDestination
        {
            get
            {
                if ( eVerticalTransferOrder == VerticalTransferOrder.UNKNOWN ||
                     eHorizontalTransferOrder == HorizontalTransferOrder.UNKNOWN )
                {
                    return FirstPixelDestination.UNKNOWN;
                }
                else if ( eVerticalTransferOrder == VerticalTransferOrder.BOTTOM &&
                          eHorizontalTransferOrder == HorizontalTransferOrder.LEFT )
                {
                    return FirstPixelDestination.BOTTOM_LEFT;
                }
                else if ( eVerticalTransferOrder == VerticalTransferOrder.BOTTOM &&
                          eHorizontalTransferOrder == HorizontalTransferOrder.RIGHT )
                {
                    return FirstPixelDestination.BOTTOM_RIGHT;
                }
                else if ( eVerticalTransferOrder == VerticalTransferOrder.TOP &&
                          eHorizontalTransferOrder == HorizontalTransferOrder.LEFT )
                {
        ...
        /// </summary>
        public List < Color > ColorMap => cColorMap;
        ...
        protected internal void SetColorMapType( ColorMapTypes eColorMapType )
        {
    ...
    internal class TargaExtensionArea
    {
        ...
        /// </summary>
        public List < int > ScanLineTable => intScanLineTable;
        ...
        /// </summary>
        public List < Color > ColorCorrectionTable => cColorCorrectionTable;
