...
namespace CSharpImageLibrary
{
    ...
    public enum ImageEngineFormat
    {
        ...
        /// </summary>
        [Description(
                        "(BC2) Block Compressed Texture. Compresses 4x4 texels. Used for Sharp Alpha. Premultiplied alpha. "
                    )]
        DDS_DXT2 = 0x32545844,
        ...
        /// </summary>
        [Description(
                        "(BC3) Block Compressed Texture. Compresses 4x4 texels. Used for Gradient Alpha. Premultiplied alpha."
                    )]
        DDS_DXT4 = 0x34545844,
        ...
        /// </summary>
        [Description(
                        "(BC4) Block Compressed Texture. Compresses 4x4 texels. Used for Normal (bump) Maps. 8 bit single channel with optional 1bit alpha."
                    )]
        ...
        /// </summary>
        [Description(
                        "(BC5) Block Compressed Texture. Compresses 4x4 texels. Used for Normal (bump) Maps. Pair of 8 bit channels."
                    )]
        ...
        /// <summary>
        ///     Used when the exact format is not present in this enum, but enough information is present to load it. (ARGB16 or
        ///     something)
        /// </summary>
        [Description(
    ...
    public partial class ImageFormats
    {
        ...
        public enum SupportedExtensions
        {
            ...
            /// <summary>
            ///     DirectDrawSurface image. DirectX image, supports mipmapping, fairly poor compression/artifacting. Good for video
            ///     memory due to mipmapping.
            /// </summary>
            ...
            /// <summary>
            ///     Graphics Interchange Format images. Lossy compression, supports animation (this tool doesn't though), good for low
            ///     numbers of colours.
            /// </summary>
        ...
        /// </summary>
        public static List < ImageEngineFormat > SaveUnsupported =
            new List < ImageEngineFormat >()
            {
                ImageEngineFormat.TGA,
        ...
        public static ImageEngineFormat FindFormatInString( string stringWithFormatInIt )
        {
            ...
            foreach ( string formatName in Enum.GetNames( typeof( ImageEngineFormat ) ) )
            {
                ...
                {
                    check = stringWithFormatInIt.Contains( "3dc", StringComparison.OrdinalIgnoreCase ) ||
                            stringWithFormatInIt.Contains( "ati2", StringComparison.OrdinalIgnoreCase );
                }
                else if ( actualFormat == "A8L8" )
                {
                    check = stringWithFormatInIt.Contains( "L8", StringComparison.OrdinalIgnoreCase ) &&
                            !stringWithFormatInIt.Contains( "G", StringComparison.OrdinalIgnoreCase );
                }
                else if ( actualFormat == "G8_L8" )
                {
                    check = !stringWithFormatInIt.Contains( "A", StringComparison.OrdinalIgnoreCase ) &&
                            stringWithFormatInIt.Contains( "G8", StringComparison.OrdinalIgnoreCase );
                }
                else if ( actualFormat.Contains( "ARGB" ) )
                {
                    check = stringWithFormatInIt.Contains( "A8R8G8B8", StringComparison.OrdinalIgnoreCase ) ||
                            stringWithFormatInIt.Contains( "ARGB", StringComparison.OrdinalIgnoreCase );
                }
        ...
        public static int GetCompressedSize(
            int numMipmaps,
            ImageEngineFormatDetails formatDetails,
            int width,
            int height )
        {
        ...
        public static List < string > GetSupportedExtensions( bool addDot = false )
        {
            if ( addDot )
            {
                return Enum.GetNames( typeof( SupportedExtensions ) ).
                            Where( t => t != "UNKNOWN" ).
                            Select( g => "." + g ).
                            ToList();
            }
        ...
        public static List < string > GetSupportedExtensionsDescriptions()
        {
            ...
            List < string > names = GetSupportedExtensions();

            foreach ( string name in names )
            {
                SupportedExtensions enumValue =
                    ( SupportedExtensions )Enum.Parse( typeof( SupportedExtensions ), name );

                descriptions.Add( General.GetEnumDescription( enumValue ) );
            }
        ...
        public static List < string > GetSupportedExtensionsForDialogBox()
        {
            ...
            foreach ( string name in names )
            {
                ...
                string desc = General.GetEnumDescription( enumValue );
        ...
        ///     Optionally list of supported extensions. Good if looping and can initialise supported and pass
        ///     into this every loop.
        /// </param>
        ...
        private static int GetBlockSize( ImageEngineFormat format, int componentSize = 1 )
        {
            int blocksize = 1;
        ...
        private static string GetExtensionOfFormat( ImageEngineFormat format )
        {
        ...
        private static bool IsBlockCompressed( ImageEngineFormat format )
        {
            switch ( format )
            {
                case ImageEngineFormat.DDS_ATI1:
        ...
        /// <returns></returns>
        private static bool IsFormatMippable( ImageEngineFormat format )
        {
        ...
        private static int MaxNumberOfChannels( ImageEngineFormat format )
        {
            int numChannels = 4;
